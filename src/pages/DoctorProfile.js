...
const DoctorDetailsPage = (props) => {
    ...
    const fetchDoctorReviews = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/api/doctors/${doctorId}/fetchreviews`
        );
        if (response.ok) {
          const data = await response.json();
          setReviews(data);
  ...
  const handleReviewSubmit = async () => {
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/doctors/${doctorId}/reviews`,
        {
          method: "POST",
          headers: {
        ...
        <div className="row doctorProfile bg-white py-4">
          <div className="col-md-4 d-flex justify-content-center align-items-center">
            <img
              ...
              alt={doctorData.name}
              className="img-fluid rounded-circle mb-4 mx-5"
            />
          </div>
          <div className="col-md-8 doctorProfileDesc px-5">
            ...
            <p
              ...
              style={{
                color:
                  doctorData.isAvailable === "Available"
                    ? "green"
                    : doctorData.isAvailable === "Not Available"
                    ? "red"
                    : "blue",
              }}
            >
            ...
            <p className="mb-1">
              <strong>Contact :</strong> {doctorData.email}
            </p>
            <p className="mb-4">
