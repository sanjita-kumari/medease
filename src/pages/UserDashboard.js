...
const UserDashboard = (props) => {
  ...
  const fetchUpdatedSessions = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/api/sessions?userId=${userDetails._id}&userType=${userDetails.userType}`,
        {
          method: "GET",
          headers: {
  ...
  const handleAvailabilityChange = async (newAvailability) => {
    try {
      const response = await fetch(
        `${BASE_URL}/api/doctors/${userDetails._id}/update-availability`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAcceptSession = async (sessionId) => {
    try {
      const response = await fetch(
        `${BASE_URL}/api/sessions/${sessionId}/accept`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleRejectSession = async (sessionId) => {
    try {
      const response = await fetch(
        `${BASE_URL}/api/sessions/${sessionId}/reject`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="container my-3 py-2">
      <div className="row doctorProfile bg-white py-4">
        <div className="col-md-4 d-flex justify-content-center align-items-center">
          <img
            ...
            className="img-fluid rounded mb-4 mx-5"
          />
        </div>
        {userDetails.userType === "doctor" ? (
          <div className="col-md-8 doctorProfileDesc px-5">
            ...
            <p className="mb-1">
              <strong>Contact :</strong> {userDetails.email}
            </p>
            <p className="mb-1">
            ...
            <p
              ...
              style={{
                color:
                  userDetails.isAvailable === "Available"
                    ? "green"
                    : userDetails.isAvailable === "Not Available"
                    ? "red"
                    : "blue",
              }}
            >
            ...
            <div className="mb-3">
              ...
              >
                ...
                <option value="Will be available soon">
      ...
      {userDetails.userType === "doctor" && (
        <div className="d-flex justify-content-center mt-3">
          <button className="btn btn-primary" onClick={toggleSessionRequests}>
            Session Requests
          </button>
        </div>
      )}
              ...
              <div key={session._id} className="card mt-3">
                <div className="card-body">
                  ...
                  <p className="card-text text-muted mt-2 mb-1">
                    <strong>Problem:</strong> {session.problem || "N/A"}
                  </p>
                  <p className="card-text text-muted">
                    <strong>Symptoms:</strong> {session.symptoms || "N/A"}
                  </p>
            ...
            .map((session, index) => (
              <div
                ...
                className={`session-card ${
                  session.status === "Active" ? "active-session" : ""
                }`}
              >
                <div
                  className="card mb-4"
                ...
                >
                  <div className="card-body">
                    <div className="d-flex justify-content-between">
                      <h6 className="card-title" style={{ color: "#61677A" }}>
                        Session - {index + 1}
                      </h6>
